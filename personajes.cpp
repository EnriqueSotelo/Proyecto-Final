    ...
    for(int i = 0; i < 5; i++){
        ...
        for (int j = 0; j < 11; j++ ){
            ...
            E[indice].inicia("Recursos/enemigos.bmp", "Recursos/Bala_Enem.bmp", 6, 12, 25, 20, 140+j*30, 130+i*24, 8,_tipo,1,
                             "Recursos/Disparo.wav", "Recursos/Explosion.wav");
        }
    }
}
...
void pintar_escudos(struct escudo ES[], BITMAP* img_mur, BITMAP* bufffer){
    for(int i = 0; i < 20; i++){
        if(ES[i].dan !=3)
        masked_blit(img_mur, bufffer, ES[i].dan*20, ES[i].tipo*16, ES[i].x, ES[i].y, 20,16 );
